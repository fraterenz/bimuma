...
class BiMuMa:
    def __init__(self, matrix: pd.DataFrame) -> None:
        ...
        self.polytomies = matrix.T.duplicated().sum()
...
def filter_cells_from_matrix(binary_mut: BiMuMa, cell2keep) -> BiMuMa:
    data = binary_mut.matrix
    data = data.drop(columns=data.loc[:, ~data.columns.isin(cell2keep)].columns)
    assert data.shape[1], "Dropped all cells"
    return BiMuMa(data)


def compute_sfs(binary_mut: BiMuMa) -> Sfs:
...
def compute_burden(binary_mut: BiMuMa) -> Burden:
